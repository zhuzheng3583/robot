...
int8_t call_c_reg_read(struct Mpu6000 *p, uint8_t reg, uint8_t len, uint8_t *buf);
int8_t call_c_reg_write(struct Mpu6000 *p, uint8_t reg, uint8_t len, uint8_t *buf);
void call_c_mdelay(struct Core *p, uint32_t ms);


/* The following functions must be defined for this platform:
 * i2c_write(U8 slave_addr, U8 reg_addr, U8 length, U8 *data)
 * i2c_read(U8 slave_addr, U8 reg_addr, U8 length, U8 *data)
...
#define i2c_read(_addr, _reg, _len, _buf) 	call_c_reg_read(NULL, _reg, _len, _buf)
#define i2c_write(_addr, _reg, _len, _buf)	call_c_reg_write(NULL, _reg, _len, _buf)
#define delay_ms(ms)    					call_c_mdelay(NULL, ms)
#define get_ms
#define reg_int_cb
#define log_i     							INF
#define log_e     							ERR
#define fabs        fabsf		// labs is already defined by TI's toolchain.
#define min(a,b) ((a<b)?a:b)	// fabs is for doubles. fabsf is for floats.
#endif
...
#if defined MPU6050
#if 1 // add by zhuzheng start
	if (i2c_read(st.hw->addr, st.reg->who_am_i, 1, data))
		return -1;
    if(data[0] != 0x68) {
        // 器件ID不正确
        log_e("Failed to read mpu6000 ID...\n");
        return -1;
    }
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    if (fifo_count < packet_size) {
		log_i("mpu fifo under flow, count: %hd.\n", fifo_count);
        return -1;
	}
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        ...
        if (i2c_read(st.hw->addr, st.reg->int_status, 1, data))
            return -1;
		//log_i("mpu fifo count: %hd.\n", fifo_count);
        if (data[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
			log_i("mpu fifo over flow, count: %hd.\n", fifo_count);
            return -2;
        }
    }
